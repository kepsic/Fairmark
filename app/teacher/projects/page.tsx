  ...
  return (
    <div className="p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-2 text-[#003A79]">
            Teacher Dashboard
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white border border-gray-200 shadow-sm rounded-lg p-4 mb-4">
            ...
            <p className="text-3xl font-bold text-[#003A79]">{groups.length}</p>
          </div>
          <div className="bg-white border border-gray-200 shadow-sm rounded-lg p-4 mb-4">
            <p className="text-sm text-gray-600 mb-1">Total Students</p>
            <p className="text-3xl font-bold text-[#005BB5]">
              {groups.reduce((sum, g) => sum + g.members.length, 0)}
            </p>
          </div>
          <div className="bg-white border border-gray-200 shadow-sm rounded-lg p-4 mb-4">
            <p className="text-sm text-gray-600 mb-1">Total Tasks</p>
            <p className="text-3xl font-bold text-[#003A79]">
        ...
        {/* Projects List */}
        <div>
          <h2 className="text-xl font-semibold mb-4 text-[#003A79]">All Projects</h2>
          {groups.length === 0 ? (
            <div className="bg-white border border-gray-200 shadow-sm rounded-lg p-8 mb-4 text-center">
              <p className="text-[#333333]">No projects yet.</p>
            </div>
          ) : (
              ...
              {groups.map((group) => {
                ...
                return (
                  <Link key={group.id} href={`/teacher/project/${group.id}`}>
                    <div className="bg-white border border-gray-200 shadow-sm rounded-lg p-4 mb-4 hover:shadow-md transition-shadow cursor-pointer">
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          <h3 className="text-lg font-semibold text-[#003A79]">{group.name}</h3>
                          {group.description && (
